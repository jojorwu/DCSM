...
*   **Основные функции (текущая базовая реализация):**
    ...
    *   **Подписка на события:** Метод `SubscribeToSWMEvents` позволяет агентам подписываться на события создания, обновления и вытеснения КЕП в SWM. SWM использует внутренние очереди для каждого подписчика и серверный стриминг для доставки событий. Реализована базовая фильтрация событий на стороне сервера.
    *   **Управление распределенными блокировками**: SWM предоставляет механизмы для получения и освобождения блокировок на именованные ресурсы, позволяя агентам координировать доступ к общим данным или КЕП.
        *   **Захват с ожиданием**: Метод `AcquireLock` поддерживает параметр `timeout_ms`, позволяющий агенту ожидать освобождения ресурса на стороне сервера в течение указанного времени.
        *   **Аренда блокировок (Lease)**: Блокировки могут быть получены на определенное время (`lease_duration_ms`).
        *   **Автоматическая очистка**: SWM включает фоновый процесс, который периодически проверяет и удаляет блокировки с истекшим временем аренды, предотвращая "зависание" ресурсов.
        *   **Уведомление ожидающих**: При освобождении или истечении блокировки, ожидающие агенты уведомляются для повторной попытки захвата.
    *   **Управление распределенными счетчиками**: SWM позволяет создавать, атомарно инкрементировать/декрементировать и получать значения именованных счетчиков, которые могут использоваться агентами для различных задач координации или сбора статистики. Счетчики хранятся в памяти SWM.
*   **Стратегии кэширования и вытеснения в SWM**:
    *   **Хранилище**: Используется `IndexedLRUCache` (см. раздел 3.6), который реализует LRU (Least Recently Used) стратегию вытеснения.
    *   **Вытеснение**: Когда кэш достигает максимального размера (`SWM_CACHE_MAX_SIZE`), добавление нового элемента приводит к вытеснению наиболее давно неиспользованного элемента.
    *   **Уведомление о вытеснении**: При вытеснении КЕП из кэша, SWM генерирует событие `KEM_EVICTED` и уведомляет об этом всех заинтересованных подписчиков. Это позволяет агентам или другим системам реагировать на удаление КЕП из оперативной памяти.
*   **Синхронизация SWM с GLM**:
    *   **Загрузка из GLM (`LoadKEMsFromGLM`)**: SWM может по запросу загружать КЕП из GLM. Загруженные КЕП помещаются в кэш SWM. Если КЕП с таким ID уже была в кэше, она обновляется. При помещении в кэш генерируются события `KEM_PUBLISHED` или `KEM_UPDATED`.
    *   **Сохранение в GLM (`PublishKEMToSWM`)**: При публикации КЕП в SWM, если установлен флаг `persist_to_glm_if_new_or_updated`, SWM пытается сохранить или обновить эту КЕП в GLM.
        *   Если GLM успешно сохраняет/обновляет КЕП и возвращает ее (возможно, с обновленными серверными временными метками или сгенерированным ID), SWM **обновляет соответствующую КЕП в своем кэше этой версией из GLM**. Это обеспечивает большую консистентность данных между SWM и GLM.
*   **Технологии (текущая реализация):**
    *   gRPC для API.
    *   `IndexedLRUCache` (на базе `cachetools.LRUCache`): для внутреннего кэша с индексацией.
    *   GLM gRPC клиент (с retry-логикой).
    *   `queue.Queue` для управления событиями подписчиков.
...
#### 4.3.1. Структура данных в SWM (Кэш)

*   **Назначение**: Быстрое хранение "горячих" КЕП для оперативного доступа агентами.
*   **Хранилище**: `IndexedLRUCache` (потокобезопасный LRU-кэш с возможностью индексации по ключам метаданных).
*   **Формат КЕП**: КЕП хранятся в кэше как экземпляры proto-сообщений `kem_pb2.KEM`. Это обеспечивает консистентность данных с форматом, используемым при gRPC-обмене и в GLM.
*   **Индексация**:
    *   Основные данные КЕП доступны по `id` через стандартные методы LRU-кэша.
    *   Дополнительно, `IndexedLRUCache` строит внутренние индексы по значениям ключей метаданных, указанных в конфигурации SWM (`SWM_INDEXED_METADATA_KEYS`). Эти индексы позволяют быстро находить ID КЕП, соответствующих определенным значениям метаданных, без полного сканирования кэша (используется в `QuerySWM`).
*   **Пример**: Если `SWM_INDEXED_METADATA_KEYS="topic,status"`, то для КЕП с `metadata={"topic":"AI", "status":"draft"}` будут обновлены индексы по `topic="AI"` и `status="draft"`.
