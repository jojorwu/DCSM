...
class GLMClient:
    def __init__(self, server_address='localhost:50051',
                 ...
                 retry_jitter_fraction: float = DEFAULT_JITTER_FRACTION, # Added
        ...
        self.channel: Optional[grpc.Channel] = None # Type hint
        self.stub: Optional[glm_service_pb2_grpc.GlobalLongTermMemoryStub] = None # Type hint
        ...
        if not self.channel:
            try:
                if self.tls_enabled:
                    ...
                    logger.info(f"GLMClient: Secure channel created for {self.server_address}.")
                else:
                    self.channel = grpc.insecure_channel(self.server_address)
                    logger.info(f"GLMClient: Insecure channel created for {self.server_address}.")

                self.stub = glm_service_pb2_grpc.GlobalLongTermMemoryStub(self.channel)
                logger.info(f"GLMClient connected to: {self.server_address} (TLS: {self.tls_enabled})")

            except grpc.FutureTimeoutError:
                logger.error(f"GLMClient: Failed to connect to GLM service at {self.server_address} within timeout (TLS: {self.tls_enabled}).")
                self._reset_connection_state()
            except FileNotFoundError as e_fnf:
                logger.error(f"GLMClient: TLS certificate/key file not found: {e_fnf} (TLS: {self.tls_enabled}). Connection failed.")
                self._reset_connection_state()
            except Exception as e:
                logger.error(f"GLMClient: Error connecting to GLM service {self.server_address} (TLS: {self.tls_enabled}): {e}", exc_info=True)
                self._reset_connection_state()
